...
using NinjaCSharpAssemblyTargetWriterTest = TestWithScheduler;

TEST_F(NinjaCSharpAssemblyTargetWriterTest, RustExecutable) {
  Err err;
  TestWithScope setup;

  Target target(setup.settings(), Label(SourceDir("//foo/"), "bar"));
  target.set_output_type(Target::CSHARP_ASSEMBLY);
  target.visibility().SetPublic();
  SourceFile main("//Program.cs");
