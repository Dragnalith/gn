...
using CSharpFunctionsTarget = TestWithScheduler;

// Checks that the appropriate crate type is used.
TEST_F(CSharpFunctionsTarget, AssemblyBase) {
  TestWithScope setup;
  ...
  TestParseInput assembly_input(
      "csharp_assembly(\"foo\") {\n"
      "  sources = [ \"Program.cs\", \"MyClass.cs\", \"Properties/AssemblyInfo.cs\" ]\n"
      "}\n");
  ASSERT_FALSE(assembly_input.has_error());
  Err err;
  assembly_input.parsed()->Execute(setup.scope(), &err);
